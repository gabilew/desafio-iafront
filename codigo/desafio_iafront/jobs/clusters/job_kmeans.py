...
@click.option('--n-samples', default=1.0, help='percentual amostras a serem utilizadas para clsuterização')
def main(dataframe: str, number_of_cluster: int, saida: str, data_inicial, data_final, drop_departamentos, n_samples):
    ...
    dataframe = read_partitioned_json(file_path=dataframe, filter_function=filter_function)
        ...
        #essa informação não será salva  para análises futuras, por exemplo, checar se mesmas categorias pertencem ao mesmo cluster
