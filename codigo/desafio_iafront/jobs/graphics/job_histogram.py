...
def main(dataframe_path: str, saida: str, x_axis, y_axis, cluster_label, data_inicial, data_final, transform):
    ...
    p1 = hist(dataframe, x_axis,   title="Original")
    p2 = hist(dataframe, x_axis+"_transformed",  title=transform)

    convertido = dataframe[dataframe.convertido==1]
    p3 = hist(convertido, x_axis,   title="Original-Convertido")
    p4 = hist(convertido, x_axis+"_transformed",  title=transform+"-Convertido")
